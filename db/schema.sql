...
CREATE TABLE IF NOT EXISTS Category (
    ...
    FOREIGN KEY (Parent_Category_ID) REFERENCES Category (Category_ID) ON UPDATE CASCADE ON DELETE SET NULL) ENGINE = InnoDB;  -- Ensure you use InnoDB which supports FULLTEXT indexes

ALTER TABLE Category ADD FULLTEXT idx_category_ngram (Category_Name) WITH PARSER ngram;
...
CREATE TABLE IF NOT EXISTS Items (
    ...
    Category_ID INT,
    PRIMARY KEY (Item_ID),
    FOREIGN KEY (Category_ID) REFERENCES Category (Category_ID) ON UPDATE CASCADE ON DELETE SET NULL) ENGINE = InnoDB; -- Make sure you use a storage engine that supports FULLTEXT indexes, such as InnoDB

ALTER TABLE Items ADD FULLTEXT INDEX idx_items_ngram (brand, name) WITH PARSER ngram;
...
CREATE TABLE IF NOT EXISTS EndUser (
    ...
    PASSWORD VARCHAR(255) NOT NULL,
    salt VARCHAR(255) NOT NULL,
    user_type ENUM ('buyer',
                    'seller') NOT NULL DEFAULT 'buyer',
    PRIMARY KEY (User_Id)
    );
...
CREATE TABLE IF NOT EXISTS Auction (
    ...
    FOREIGN KEY (Winner) REFERENCES EndUser (User_Id) ON UPDATE CASCADE ON DELETE SET NULL
    );
...
CREATE TABLE IF NOT EXISTS Alerts (
    ...
    Status ENUM ('Unread',
                 'Read',
                 'Pending') NOT NULL DEFAULT 'Unread',
    Auction_ID INT,
